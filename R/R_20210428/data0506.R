...
ozone1<- airquality
sum(ozone1$ozone,na.rm = T)
mean(ozone1$ozone,na.rm = T)
sd(ozone1$ozone,na.rm = T)
var(ozone1$ozone,na.rm = T)
...
# 2. 모든 NA 제거 후, 각 월별로 측정값 들의 평균

ozone2<-airquality
ozone2
sum(subset(ozone2,month<=5)$ozone,na.rm = T)
sum(subset(ozone2,month>5&month==6)$ozone,na.rm = T)
sum(subset(ozone2,month>6&month==7)$ozone,na.rm = T)
sum(subset(ozone2,month>7&month==8)$ozone,na.rm = T)
sum(subset(ozone2,month>8&month==9)$ozone,na.rm = T)
...
ozone3<-airquality
ozone3
mean(subset(ozone3,month<=5)$solar.r,na.rm = T)  
mean(subset(ozone3,month<=5)$wind,na.rm = T)    
...
str(airquality)
class(airquality)
sum(subset(ozone4,month<=5)$ozone,na.rm = T)
mean(subset(ozone4,month<=5)$ozone,na.rm = T)
var(subset(ozone4,month<=5)$ozone,na.rm = T)
sd(subset(ozone4,month<=5)$ozone,na.rm = T)

# 샘
ozone5 <- subset(airquality, month==5, select=ozone)
class(ozone5)
onone5$ozone
mean(onone5$ozone, na.rm = T)
...
library(dplyr)
fil_data <- filer(airquality, airquality$month==5)
fil_data
sel_data <- select(fil_data, ozone)
class(sel_data)
mean(sel_data, na.rm = T)

airquality %>% filter(month==5) %>% select(ozone) %>%
...
ozone8<-airquality
MAX2<-max(ozone8$solar.r,na.rm = T)
subset(ozone8,solar.r==MAX2)
