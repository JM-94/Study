...
hist(exdata1$AGE, main='계좌이용 나이 분포', xlab='나이 구간',
     ylab='나이 빈도', xlim=c(1,100),col='green',probability=T)
hist(exdata1$AGE, main='나이 분포')
hist(exdata1$AGE, main='계좌이용 나이 분포', xlab='나이 구간',
     ylab='나이 빈도', col='green',freq=T, breaks=3)
...
# 박스내의 가로선 : 중앙값
# lower whisker : 최소값
# -> 중앙값 -1.5 X IQP보다 큰 데이터 중 가장 작은 값
# upper whisker : 최대값
# -> 중앙값 +1.5 X IQP보다 작은 데이터 중 가장 큰 값
# IQR (Inter Quartile Range)
# -> Q3(제3사분위수) - Q1(1사분위수) = IQR
# 점 : 이상값(outlier) - 특이점
...
par(mfrow=c(1,2))
hist(iris$Sepal.Width,xlab='꽃받침 길이',
     col='yellow',
