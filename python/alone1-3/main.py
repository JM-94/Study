...
@app.route("/")
def home():
    wlength = request.args.get("length", default=30, type=float)
    wweight = request.args.get('weight', default=600, type=float)
    # 도미 데이터
    ...
    plt.scatter(bream_length, bream_weight, marker='o')  # 도미데이터그리기
    plt.scatter(smelt_length, smelt_weight, marker='*')  # 빙어데이터그리기
    ...
    svalue = knc.score(fish_data, fish_target)  # 이 데이터를 넣을때 알고리즘 확률
